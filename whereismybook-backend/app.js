...
app.post("/api/categoria", async (req, res) => {
    try {
        ...
        //Envio la info:
        res.send({ "respuesta": "ID: " + respuesta.insertId + ", nombre: " + nombre });
    }
    catch (e) {
...
app.delete('/api/categoria/:id', async (req, res) => {
    try {
        ...
        res.send('La categoría se eliminó correctamente');
        res.status(200);
...
app.put('/api/persona/:id', async (req, res) => {
    ...
    try {
        ...
        if (consulta.length == 1) {
            ...
            await qy('UPDATE persona SET nombre=?, apellido=?, alias=? WHERE id=?', [nombre, apellido, alias, req.params.id]);
